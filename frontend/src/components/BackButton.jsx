...
export const BackButton = ({ id: userId, post: postId, state = {} }) => {
  ...
  const getUserPhoto = () => {
    return userdata?.user?.photo || userdata?.photo || assets.user_image;
  };

  const getUserName = () => {
    return userdata?.user?.name || userdata?.name || 'User';
  };
  ...
  const getIsAccountVerified = () => {
    return userdata?.isAccountVerified || userdata?.user?.isAccountVerified || false;
  };
  ...
  const logout = async () => {
    try {
      ...
      if (data.success) {
        ...
        setUserData(false);
        navigate("/");
        toast.success(`${getUserName()} logged out successfully`);
      }
    } catch (error) {
      toast.error(error.response?.data?.message || "Logout failed");
    }
  ...
  // Combine prop state and location state (prop state has priority)
  const navigationState = { ...location.state, ...state };
  console.log("Combined state:", navigationState);

  // Check current path
  const currentPath = location.pathname;

  // Check for specific navigation keys
  if (navigationState.verificationKey === "product_view_key_123") {
  ...
  else if (navigationState.verifiyKey === "product_back_123") {
  ...
  else if (navigationState.fromHome) {
    // Came from home page
    navigate('/home');
    return;
  }
  else if (navigationState.fromPosts) {
    // Came from posts page
    navigate('/home');
    return;
  }
  // else if (navigationState.verificationKey === "profile_view_key_123") {
  //   // Came from profile click in posts
  //   navigate('/home');
  //   return;
  // }
  
  else if (navigationState.verificationKey === "author_from_view_detail_123") {
    // Came from author click in view-detail
    if (navigationState.viewDetailPath) {
      navigate(navigationState.viewDetailPath);
    } else {
      navigate(-1);
    }
    return;
  ...
  // If no verification key and we're in view-detail, go to home
  if (currentPath.includes('/view-detail/')) {
    console.log("No verification key found and in view-detail, navigating to home");
    navigate('/home');
    return;
  }

  // If no verification key and we're in view-users, go to home
  if (currentPath.includes('/view-users/')) {
    console.log("No verification key found and in view-users, navigating to home");
    navigate('/home');
    return;
  }
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ease-in-out ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        {/* Logo as Back Button - Only this acts as back navigation */}
        <div className="flex items-center">
          <img
            src={assets.logo}
            alt="Logo"
            className={`h-9 transition-all duration-300 ${isScrolled ? 'scale-95' : 'scale-100'} cursor-pointer hover:scale-105`}
