...
import { FaUser, FaSignOutAlt, FaEnvelope, FaChevronDown, FaCheckCircle, FaShieldAlt } from 'react-icons/fa';
import { assets } from '../../assets/assets';
import { AppContext } from '../../context/AppContext';

export const AdminNavbar = ({state = {}}) => {
  const [isScrolled, setIsScrolled] = useState(false);
  const navigate = useNavigate();
  const [showMenu, setShowMenu] = useState(false);
  const { userdata, backendUrl, setIsLogged, setUserData, isAdmin } = useContext(AppContext);
  ...
  const logout = async () => {
    try {
      ...
      if (data.success) {
        ...
        setUserData(false);
        navigate("/");
        toast.success(`${userdata.user?.name} logged out successfully`);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const handleBackAdmin = () => {
      navigate('/admin', { state: navigationState });
    }
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ease-in-out ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        {/* Admin Dashboard Button - Visible on desktop for admin */}
              ...
              <div className="absolute right-0 mt-3 w-64 origin-top-right bg-white rounded-xl shadow-xl ring-1 ring-gray-200 focus:outline-none overflow-hidden animate-fadeIn">
                <div className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 border-b">
                  <div className="flex items-center mb-3">
                    ...
                    <div className="ml-3">
                      <p className="text-sm font-semibold text-gray-900">{userdata.user?.name}</p>
                ...
                <div className="p-2">
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="flex w-full items-center px-4 py-3 text-sm text-left text-gray-700 hover:bg-purple-50 rounded-lg transition-colors mb-1"
                    >
