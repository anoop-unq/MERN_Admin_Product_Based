...
export const AppContextProvider = (props) => {
  ...
  // In your AppContext.js, update the admin functions:
...
const getAdminStats = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      toast.error(response.data.message || "Failed to fetch admin stats");
    }
  } catch (error) {
    toast.error(
...
const getAllPostsAdmin = async () => {
  ...
  try {
    setIsAdminLoading(true);
    const response = await axios.get(`${backendUrl}/api/admin/posts`, {
      withCredentials: true,
    });
...
const deletePostAdmin = async (postId) => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      // Update both admin posts and regular posts
...
// In your AppContext.js - Add logout function
...
const logout = async () => {
  ...
  const getAllComments = async () => {
    ...
    try {
      setIsAdminLoading(true);
      const response = await axios.get(`${backendUrl}/api/admin/comments`, {
        withCredentials: true,
      });
  ...
  // Update your existing getAuthState to handle role
  ...
  // ... your existing post functions (createPost, updatePost, deletePost, etc.) remain the same ...
...
// In your AppContext.js - Fix the API endpoints

const register = async (userData) => {
  try {
    const response = await axios.post(
      `${backendUrl}/api/register`,  // Changed from /api/auth/register to /api/register
      userData,
      { 
        withCredentials: true,
        headers: {
...
const login = async (email, password) => {
  try {
    const response = await axios.post(
      `${backendUrl}/api/login`,  // Changed from /api/auth/login to /api/login
      { email, password },
      { withCredentials: true }
    );
