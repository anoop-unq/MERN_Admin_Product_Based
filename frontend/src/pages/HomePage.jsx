...
import PostList from '../components/PostList';
import SearchResults from '../components/SearchResults';
import { FaArrowLeft, FaSearch, FaTimes, FaHome, FaCrown, FaUsers, FaChartBar } from 'react-icons/fa';
import debounce from 'lodash/debounce';
import { assets } from '../assets/assets';
...
const HomePage = () => {
  ...
  const { verifiyAdminBack } = location.state || {};
  ...
  const handleBackAdminOrHome = () => {
    if (verifiyAdminBack && isAdmin) {
      navigate('/admin');
    } else {
      navigate('/');
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 font-['Inter']">
            ...
            {isAdmin && (
              <div className="flex items-center space-x-2 ml-4">
                <button
                  ...
                  className="flex items-center space-x-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105"
                >
              ...
              <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-2xl p-6 shadow-lg">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="text-xl font-bold">Welcome, Admin {userdata?.user?.name}!</h3>
                      <p className="text-purple-100">Manage users, posts, and platform analytics</p>
                    </div>
                  </div>
                  <button
                    onClick={() => navigate('/admin')}
      ...
      {/* Admin Quick Actions */}
      {isAdmin && (
        <div className="fixed bottom-24 right-8 z-20 md:hidden">
          <button
            onClick={() => navigate('/admin')}
            className="flex items-center justify-center bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-2xl p-4 w-16 h-16 hover:scale-110 hover:shadow-2xl hover:shadow-purple-500/40 transition-all duration-300 shadow-lg shadow-purple-500/25"
