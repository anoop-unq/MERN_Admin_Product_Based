...
// export const register = async (req, res) => {
//   try {
//     const { name, email, password, bio, role = 'user' } = req.body; // Allow role selection
...
//       photo: photoUrl,
//       bio: bio || '',
//       role // This will be user-selected or default to 'user'
//     });
...
// // Add these admin functions to your userController.js
...
//     // Get user's posts to delete images from Cloudinary
//     const userPosts = await Post.find({ author: userId });
    
//     // Delete images from Cloudinary for each post
...
//     res.json({
//       success: true,
//       message: "User deleted successfully. All posts, comments, and likes have been removed."
//     });
//   } catch (error) {
...
// // Admin: Get all posts (admin only)
...
// // Admin: Delete any post (admin only)
...
export const updateUserRole = async (req, res) => {
  try {
    ...
    // Prevent changing admin users to regular users
    if (targetUser.role === 'admin' && role === 'user') {
      return res.status(403).json({
        success: false,
        message: "Cannot change admin role to user"
      });
    }
